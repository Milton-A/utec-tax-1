...
router.get('/totalCliente/:id', async (req, res) => {
  try {
    const viagens = await viagemController.getTotalViagensByClienteId(req.params.id);
    return res.json(viagens);
  }catch(err) {
    console.log(err);
  }
...
router.get('/totalMotorista/:id', async (req, res) => {
  try {
    const viagens = await viagemController.getTotalViagensByMotoristaId(req.params.id);
    return res.json(viagens);
  }catch(err) {
    console.log(err);
  }
